...
function renderBossBulletShape(bullet, color) {
    ...
    switch (bullet.pattern) {
        ...
        case 'heart_shot':
            ...
            ctx.beginPath();
            // 하트 모양 그리기 - 크기 50% 증가
            ctx.moveTo(0, scaledSize * 0.3 * 1.5);
            ctx.bezierCurveTo(-scaledSize * 0.5 * 1.5, -scaledSize * 0.2 * 1.5, -scaledSize * 1.5, scaledSize * 0.1 * 1.5, 0, scaledSize * 1.5);
            ctx.bezierCurveTo(scaledSize * 1.5, scaledSize * 0.1 * 1.5, scaledSize * 0.5 * 1.5, -scaledSize * 0.2 * 1.5, 0, scaledSize * 0.3 * 1.5);
            ctx.fill();
            break;
        ...
        case 'butterfly_shot':
            ...
            // 왼쪽 날개
            ctx.beginPath();
            ctx.ellipse(-scaledSize * 0.5, 0, scaledSize * 0.8, scaledSize * 0.5, -Math.PI/4, 0, Math.PI * 2);
            ctx.fill();
            // 오른쪽 날개
            ctx.beginPath();
            ctx.ellipse(scaledSize * 0.5, 0, scaledSize * 0.8, scaledSize * 0.5, Math.PI/4, 0, Math.PI * 2);
            ctx.fill();
            // 몸통
            ctx.fillRect(-scaledSize * 0.16, -scaledSize * 0.8, scaledSize * 0.32, scaledSize * 1.6);
            break;
        ...
        case 'spread_random':
            ...
            for (let i = 0; i < sides; i++) {
                ...
                const radius = size * (0.7 + Math.random() * 0.6); // 0.7-1.3배 반지름
        ...
        case 'windmill_spread':
            ...
            // 중심 원 - 크기 50% 증가
            ctx.beginPath();
            ctx.arc(0, 0, size * 0.2 * 1.5, 0, Math.PI * 2);
            ctx.fill();
            // 바람개비 날개들 - 크기 50% 증가
            for (let i = 0; i < 4; i++) {
                const angle = (i * Math.PI * 2) / 4;
                ...
                ctx.lineTo(Math.cos(angle + Math.PI/4) * size * 0.7 * 1.5, Math.sin(angle + Math.PI/4) * size * 0.7 * 1.5);
                ctx.closePath();
                ctx.fill();
            }
            break;
        ...
        case 'heart_shot':
            ...
            ctx.fillStyle = `rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, 1.0)`;
            ctx.beginPath();
            ctx.moveTo(0, size * 0.3 * 1.5);
            ctx.bezierCurveTo(-size * 0.5 * 1.5, -size * 0.2 * 1.5, -size * 0.2 * 1.5, -size * 0.5 * 1.5, 0, -size * 0.2 * 1.5);
            ctx.bezierCurveTo(size * 0.2 * 1.5, -size * 0.5 * 1.5, size * 0.5 * 1.5, -size * 0.2 * 1.5, 0, size * 0.3 * 1.5);
            ctx.closePath();
            ctx.fill();
            break;
        ...
        case 'butterfly_shot':
            ...
            // 날개들
            ctx.beginPath();
            ctx.ellipse(-size * 0.6, -size * 0.4, size * 0.6, size * 0.4, -Math.PI/6, 0, Math.PI * 2);
            ctx.fill();
            ctx.beginPath();
            ctx.ellipse(size * 0.6, -size * 0.4, size * 0.6, size * 0.4, Math.PI/6, 0, Math.PI * 2);
            ctx.fill();
            ctx.beginPath();
            ctx.ellipse(-size * 0.4, size * 0.2, size * 0.4, size * 0.3, -Math.PI/4, 0, Math.PI * 2);
            ctx.fill();
            ctx.beginPath();
            ctx.ellipse(size * 0.4, size * 0.2, size * 0.4, size * 0.3, Math.PI/4, 0, Math.PI * 2);
            ctx.fill();
        ...
        case 'moon_shot':
            ...
            ctx.fillStyle = `rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, 1.0)`;
            ctx.beginPath();
            ctx.arc(0, 0, size * 0.8, Math.PI/6, Math.PI * 11/6);
            ctx.arc(0, size * 0.2, size * 0.6, Math.PI/6, Math.PI * 11/6);
            ctx.closePath();
            ctx.fill();
            break;
            
        case 'snowflake_shot':
            // 눈 결정체 모양 총알 (크기 50% 증가)
            ctx.fillStyle = `rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, 1.0)`;
            ctx.beginPath();
            // 중심에서 6방향으로 선을 그어 눈 결정체 모양 만들기 (크기 증가)
            for (let i = 0; i < 6; i++) {
                const angle = (i * Math.PI * 2) / 6;
                const x = Math.cos(angle) * size * 1.5;  // 크기 50% 증가
                const y = Math.sin(angle) * size * 1.5;  // 크기 50% 증가
                
                if (i === 0) ctx.moveTo(0, 0);
                else ctx.moveTo(0, 0);
                ctx.lineTo(x, y);
                
                // 각 선의 중간에서 작은 가지 추가 (크기 증가)
                const midX = Math.cos(angle) * size * 0.75;  // 0.5 * 1.5
                const midY = Math.sin(angle) * size * 0.75;  // 0.5 * 1.5
                const branchAngle1 = angle + Math.PI / 3;
                const branchAngle2 = angle - Math.PI / 3;
                const branchLength = size * 0.45;  // 0.3 * 1.5
...
const BOSS_PATTERN_COLORS = {
    ...
    // 확산 패턴 색상들
    'spread_circle': '#00FFFF',           // 밝은 청록색
    'spread_cross': '#FF00FF',            // 밝은 자홍색
    'spread_spiral': '#FFFF00',           // 밝은 노란색
    'spread_diamond': '#FFA500',          // 밝은 오렌지색
    'spread_burst': '#FF69B4',            // 밝은 핫핑크
    // 새로운 모양 패턴 색상 (검정색과 대비되는 밝은색)
    'heart_shot': '#FF69B4',             // 밝은 핫핑크
    'star_shot': '#FFFF00',              // 밝은 노란색
    'flower_shot': '#FFFF00',            // 밝은 노란색
    'butterfly_shot': '#FFFF00',         // 밝은 노란색
...
const BOSS_PATTERNS = {
    ...
    // 확산 패턴들
    SPREAD_CIRCLE: 'spread_circle',        // 원형 확산 패턴
    SPREAD_CROSS: 'spread_cross',          // 십자 확산 패턴
    SPREAD_SPIRAL: 'spread_spiral',        // 나선 확산 패턴
    SPREAD_DIAMOND: 'spread_diamond',      // 다이아몬드 확산 패턴
    SPREAD_BURST: 'spread_burst',          // 폭발 확산 패턴
    SPREAD_RANDOM: 'spread_random',        // 랜덤 확산 패턴
    // 새로운 모양 패턴들
    ...
    SNOWFLAKE_SHOT: 'snowflake_shot',      // 눈 결정체 모양 패턴
...
function createBoss() {
    ...
    const allAvailablePatterns = [
        ...
        BOSS_PATTERNS.SNOWFLAKE_SHOT    // 눈 결정체 모양
    ];
    ...
    if (boss.pattern === BOSS_PATTERNS.WINDMILL_SPREAD) {
        console.log('🎯 바람개비 패턴이 선택되었습니다!');
    }
...
function handleBossPattern(boss) {
    ...
    if (currentTime - boss.lastPatternChange >= 1000) {
        ...
        if (selectedPattern === BOSS_PATTERNS.WINDMILL_SPREAD) {
            console.log('🎯 바람개비 패턴으로 패턴 변경되었습니다!');
        }
    }
    ...
    switch (pattern) {
        ...
        case BOSS_PATTERNS.SPREAD_RANDOM:
            if (currentTime - boss.lastShot >= 400) {  // 0.4초마다 발사
                // 랜덤 확산 패턴 - 랜덤한 방향으로 3-6발 발사
                const bulletCount = Math.floor(Math.random() * 4) + 3; // 3-6발
                for (let i = 0; i < bulletCount; i++) {
        ...
        case BOSS_PATTERNS.SNOWFLAKE_SHOT:
            if (currentTime - boss.lastShot >= 1200) {  // 1.2초마다 발사
                // 눈 결정체 패턴 - 6방향으로 각각 3발씩 확산 발사 (총 18발)
                for (let i = 0; i < 6; i++) {
